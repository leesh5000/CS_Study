...
- 데이터 추가 : O(n) [데이터가 추가되면 배열에 저장된 모든 데이터들과 우선순위 비교 연산을해서 자리를 찾아야한다.]
- 데이터 삭제 : O(1) [배열의 맨 앞에 저장된 데이터를 삭제하면 된다.]

#### 연결리스트 기반 우선순위 큐 ####
- 데이터 추가 : O(n) [연결리스트도 배열처럼 데이터의 우선순위를 비교 연산을해야 하므로]
- 데이터 삭제 : O(1) [맨 앞에 저장된 데이터를 삭제하면 된다.]

#### (이진)힙 기반 우선순위 큐 ####
- 데이터 추가 : O(log(n)) [데이터가 추가되면, 힙 트리의 높이만큼만 연산을 하면된다. 힙 트리의 높이와 데이터 수의 관계는 `h(n) = log<sub>2</sub>(n)`의 관계를 갖기 때문에 데이터 추가의 복잡도는 O(log(n))이다. ]
- 데이터 삭제 : O(log(n) [데이터 삭제도 추가와 마찬가지이다.)
