...
front : 연결리스트의 첫번째 노드를 나타내는 자료형
Add_after(node) : 노드를 추가하는 연산 (i는 0부터 시작한다)
Del_after(node) : 연결리스트의 i번째 위치에 노드를 삭제하는 연산
```
...
2. 원형 연결리스트 : 연결리스트의 마지막 노드가 첫 노드를 가리켜 원형으로 된 연결리스트를 말한다.
